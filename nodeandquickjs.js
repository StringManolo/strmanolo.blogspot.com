...
if (isQjs) {                                                             loadFile = std.loadFile;
} else {
  const fs = require("fs");
  loadFile = filename => {                                                 try {
      filename = fs.readFileSync(filename, { encoding: "utf-8" })
    } catch(e) {                                                             filename = null;                                                     }                                                                      return filename;                                                     };                                                                   }</code></p>
...
<p>Utilizaremos también el objeto cli para guardar nuestros argumentos con un switch: <code class="javascript">for (let i in cli.args) {
  switch(cli.args[i]) {
    ...
    case "--numbers":
      cli.number = true;
    break;                                                                                                                                        case "-s":                                                             case "--simbol":                                                       case "--simbols":                                                        cli.simbol = true;                                                   break;
...
+ -n, --numbers   Output how many numbers the file has                 + -s, --simbols   Output how many simbols the file has
...
`);                                                                        return;
    ...
    if (cli.number)                                                          console.log(cli.file.match(/[0-9]/g).length);

    if (cli.simbol)                                                          console.log(cli.file.match(/[^A-Za-z0-9]/g).length);
...
<code class="output">$ time charcounter.js -f charcounter.js -l -n -s -t
1064
9
1004
2077
